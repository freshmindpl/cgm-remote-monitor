...
function boot (env, language) {
  ...
  function setupStorage (ctx, next) {
    ...
    try {
      switch (env.storageURI.match(/^([a-z][a-z0-9+\-.]*):\/\//)[0]) {
        // _.startsWith(env.storageURI, 'openaps://')
        case 'mongodb://':
        ...
        case 'dynamodb://':
            require('../storage/dynamodb-storage')(env, function ready (err, store) {
              if (err) {
                throw err;
              }
    
              ctx.store = store;
              console.log('DynamoDB Storage system ready');
              next();
            });
        break;
        
        default:
            console.info('ERROR: Unknown storage system', env.storageURI);
            process.exit(1);
      }
    } catch (err) {
      console.info('storage err', err);
      ctx.bootErrors = ctx.bootErrors || [ ];
      ctx.bootErrors.push({'desc': 'Unable to connect to storage system', err: err});
      next();
    }
